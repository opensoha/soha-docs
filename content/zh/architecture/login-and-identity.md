...
目录连接与登录时的角色、组织映射相互独立。配置为 `scheduled_and_realtime` 的飞书连接会接收已验证的通讯录事件，并根据事件载荷应用单对象增量变更。载荷不足以提供当前数据时，Soha 只补查事件所指向的部门或人员，不拉取完整目录快照。重复投递会按幂等方式处理，较旧事件不会覆盖较新的投影。

定时或手动触发的全量对账仍作为漂移修复兜底。无法识别或载荷不完整的事件会把连接标记为需要对账，而不会让每次 webhook 都触发全量快照同步。

收到离职或人员删除事件后，Soha 会移除该目录来源拥有的成员关系。`userDisablePolicy: managed_only` 还会停用由该连接托管的账号并撤销其活动会话；`userDisablePolicy: never` 保持账号启用，只移除目录来源成员关系。缺失或删除的组织采用归档处理，不会硬删除。
