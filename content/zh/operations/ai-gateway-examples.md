...
- `approvalPolicy.approvalPolicyRef` 记录 Gateway 审批策略引用。Gateway 审批超时和路由来自同一个 inline `approvalPolicy` 对象，包括 `slaMinutes`、`approvalMode`、审批人数、候选用户/角色/团队、变更窗口和分阶段路由。
