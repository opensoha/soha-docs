...
`configs/config.compose.yaml` is the app-container config for compose; it points the database host at the `postgres` service and does not seed host-local kubeconfig paths. The optional Hermes provider runner is built from sibling repository `../soha-agent`.
